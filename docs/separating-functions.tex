...
% Generated: 2026-05-19T04:49:27.207Z
%
...
The equality function $EQ_n(x,y)$ is true iff $x_i = y_i$ for every $i \in \{1,\ldots,n\}$. It has linear-size OBDDs for interleaved variable orders, but can require exponential OBDD size for separated orders. \citep{Wegener_2000}
\end{definition}
...
The weighted sum function $WS_n$ is the standard OBDD/FBDD separation family whose output is selected by the Hamming-weight-weighted index of the input. It has polynomial-size nondeterministic OBDDs under every variable order, but requires exponential FBDD size. \citep{Wegener_2000,Savicky_Zak_1996}
\end{definition}
...
Requires that in each pair $(x_{2i}, x_{2i+1})$, at least one is true. Dual to the Pairwise DNF. \citep{Darwiche_2002}
\end{definition}

\begin{definition}[Pairwise DNF]\label{sf:pairwise-dnf}
\textbf{$\Pi_n = \bigvee_{i=0}^{n-1} \left( x_{2i} \wedge x_{2i+1} \right)$} \\
Requires that at least one pair $(x_{2i}, x_{2i+1})$ has both variables true. Dual to the Pairwise CNF. \citep{Darwiche_2002}
\end{definition}
...
$O_n = \bigoplus_{i=0}^{n-1} x_i$ has linear size $OBBD_<$ representations but exponential size CNF and DNF representations. \citep{Bryant_1986}
\end{definition}
