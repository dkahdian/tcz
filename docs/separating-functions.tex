...
% Generated: 2026-05-14T22:07:29.280Z
%
...
The equality function $EQ_n(x,y)$ is true iff $x_i = y_i$ for every $i \in \{1,\ldots,n\}$. It has linear-size OBDDs for interleaved variable orders, but can require exponential OBDD size for separated orders. \citet{Wegener_2000}
\end{definition}
...
The weighted sum function $WS_n$ is the standard OBDD/FBDD separation family whose output is selected by the Hamming-weight-weighted index of the input. It has polynomial-size nondeterministic OBDDs under every variable order, but requires exponential FBDD size. \citet{Wegener_2000,Savicky_Zak_1996}
\end{definition}
