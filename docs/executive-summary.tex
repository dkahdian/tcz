...
\subsection{Class-Level Liftability Note}
Under our class-level succinctness definition, target choice is per class member, not per formula:
...
on the specific input circuit $\mathcal{D}$, so one fixed target order per source class member is not guaranteed.
