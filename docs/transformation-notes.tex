...
\begin{enumerate}[noitemsep]
  \item Every clause can be converted into an equivalent representation of $L$ in polynomial time and polynomial size.
  \item \langref{CNF} does not compile to $L$ with polynomial blowup.
\end{enumerate}
...
Suppose a language $L$ satisfies both of the following:
\begin{enumerate}[noitemsep]
  \item Every term can be converted into an equivalent representation of $L$ in polynomial time and polynomial size.
  \item \langref{DNF} does not compile to $L$ with polynomial blowup.
\end{enumerate}
...
Compile it to \langfam{dec-SDNNF}{T}; this is automatically a \langfam{d-SDNNF}{T} representation, and hence also an \langfam{SDNNF}{T} representation.

\paragraph{\langref{SDD}, \langref{cSDD}.}
Start from an OBDD representation of the clause.
Every OBDD is a right-linear SDD \citep{Darwiche_2011}, and in fact already compressed, so this gives polynomial-size representations in both SDD and cSDD.
...
\begin{itemize}[noitemsep]
  \item a clause is already a DNNF representation, so DNNF gets a direct unconditional $\land$C lower bound from the propagated CNF $\not\leq$ DNNF gap;
...
\begin{itemize}[noitemsep]
  ...
  \item it \emph{does not} apply to nFBDD, SDNNF, nOBDD, or nOBDD$_<$, because DNF already compiles to those languages with polynomial blowup.
\end{itemize}

For the fixed-vtree classes, the upgraded statements are genuinely quasipolynomial lower bounds, not merely polynomial lower bounds: the needed succinctness gaps are already present in the map as
\[
