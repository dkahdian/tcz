<script lang="ts">
  ...
  import type { KCLanguage, LanguageVisibilityParam, ViewMode } from '$lib/types.js';
  import {
    DEFAULT_HIDDEN_LANGUAGE_IDS,
  ...
  function resetPicker() {
    ...
    onChange({
      ...value,
      mode: 'except',
      ids: DEFAULT_HIDDEN_LANGUAGE_IDS.filter((id) =>
        languages.some((language) => language.id === id)
      )
    });
  }
  ...
  function isClassLanguage(language: KCLanguage): boolean {
    return language.name.includes('$_');
  }

  function getClassIds(): string[] {
    return languages.filter(isClassLanguage).map((language) => language.id);
  }

  function getClassButtonLabel(): string {
    ...
    return `${allVisible ? 'Hide' : 'Show'} classes`;
  }
      ...
      <div class="section-header">
        ...
        {#if languagesOpen}
          <div class="section-actions">
            <button type="button" class="toolbar-btn" onclick={toggleClasses}>
              {getClassButtonLabel()}
            </button>
        ...
        {#each filteredQueries as op (op.id)}
          <label class="picker-item">
            <input
              ...
              onchange={() =>
                showGraphOperationDisplay
                  ? toggleGraphOperation('query', op.id)
                  : toggleMatrixOperation('query', op.id)}
            />
            <div class="item-copy">
        ...
        {#each filteredTransformations as op (op.id)}
          <label class="picker-item">
            <input
              ...
              onchange={() =>
                showGraphOperationDisplay
                  ? toggleGraphOperation('transformation', op.id)
                  : toggleMatrixOperation('transformation', op.id)}
            />
            <div class="item-copy">
              <span class="item-name">{op.code}</span>
              <div class="item-meta">{showGraphOperationDisplay ? `Display ${op.label} on graph nodes` : op.label}</div>
